...
const spawn = {
    ...
    blockGroupMob(x, y, radius = 25 + Math.floor(Math.random() * 20)) {
        mobs.spawn(x, y, 4, radius, color.block);
        let me = mob[mob.length - 1];
    ...
    blockMob(x, y, host, growCycles = 60) {
        if (host === null) {
            mobs.spawn(x, y, 4, 1.25 + 3.5 * Math.random(), color.block);
        } else {
            ...
            mobs.spawn(x, y, 4, Math.min(70, sideLength), color.block);
    ...
    cellBoss(x, y, radius = 20, cellID) {
        ...
        me.onDamage = function(dmg) {
            if (Math.random() < 1 * dmg * Math.sqrt(this.mass) && this.health > dmg) this.split();
        }
    ...
    spinner(x, y, radius = 30 + Math.ceil(Math.random() * 35)) {
        ...
        me.spin = function() {
            ...
            if (this.cd < simulation.cycle) {
                ...
                this.cd = simulation.cycle + 80 * simulation.CDScale
                this.do = this.look
            ...
            if (!m.isBodiesAsleep) {
                ...
                if (this.seePlayer.yes && dist2 < 4000000) {
                    ...
                    if (targetDist < r + 16) {
                        targetDist = r + 10;
                        //charge at player
                        const forceMag = this.accelMag * 80 * this.mass;
                        const angle = Math.atan2(this.seePlayer.position.y - this.position.y, this.seePlayer.position.x - this.position.x);
                        this.force.x += forceMag * Math.cos(angle);
                        this.force.y += forceMag * Math.sin(angle);
                    }
    ...
    laserTargetingBoss(x, y, radius = 80) {
        ...
        me.accelMag = 0.00036 * Math.sqrt(simulation.accelScale);
        me.seePlayerFreq = 30
        me.memory = 420;
        me.restitution = 1;
        me.frictionAir = 0.01;
        ...
        me.do = function() {
            ...
            if (this.seePlayer.recall) {
                ...
                const threshold = 0.4;
                if (c > threshold) {
                    this.torque += 0.000008 * this.inertia;
                } else if (c < -threshold) {
                    this.torque -= 0.000008 * this.inertia;
                }
                // if (Math.abs(c) < 0.3) {
    ...
    blinkBoss(x, y) {
        ...
        me.memory = 240
        me.seePlayerFreq = 60
        me.delay = 15 + 20 * simulation.CDScale;
    ...
    pulsarBoss(x, y, radius = 90) {
        ...
        me.fireDelay = Math.max(60, 150 - simulation.difficulty * 2)*0.3
        me.isFiring = false
        Matter.Body.setDensity(me, 0.01); //extra dense //normal is 0.001 //makes effective life much larger
        me.isBoss = true;
            ...
            if (!m.isBodiesAsleep) {
                if (!m.isCloak && !this.isStunned) {
                    if (this.isFiring) {
                        ...
                        if (c > threshold) {
                            this.torque += 0.000006 * this.inertia;
                        } else if (c < -threshold) {
                            this.torque -= 0.000006 * this.inertia;
                        } else if (this.fireCycle > 15) { //fire
                            unit = Vector.mult(Vector.normalise(Vector.sub(this.vertices[1], this.position)), this.distanceToPlayer() - 100)
                            this.fireTarget = Vector.add(this.vertices[1], unit)
    ...
    pulsar(x, y, radius = 40) {
        ...
        me.fireDelay = Math.max(75, 140 - simulation.difficulty * 0.5)*0.3
        me.isFiring = false
        me.onHit = function() {};
    ...
    laserBoss(x, y, radius = 30) {
        ...
        me.do = function() {
            ...
            if (!this.isStunned) {
                ...
                ctx.beginPath();
                this.laser(this.vertices[0], this.angle + Math.PI / 3*0);
                this.laser(this.vertices[1], this.angle + Math.PI / 3*1);
                this.laser(this.vertices[2], this.angle + Math.PI / 3*2);
				this.laser(this.vertices[3], this.angle + Math.PI / 3*3);
                this.laser(this.vertices[4], this.angle + Math.PI / 3*4);
                this.laser(this.vertices[5], this.angle + Math.PI / 3*5);
                ctx.strokeStyle = "#50f";
                ctx.lineWidth = 1.5;
                ctx.setLineDash([70 + 300 * Math.random(), 55 * Math.random()]);
                ctx.stroke(); // Draw it
    ...
    stabber(x, y, radius = 25 + Math.ceil(Math.random() * 12), spikeMax = 9) {
        ...
        // me.g = 0.0002; //required if using this.gravity
        me.delay = 180 * simulation.CDScale;
        me.spikeVertex = 0;
        me.spikeLength = 0;
            ...
            if (!m.isBodiesAsleep) {
                ...
                if (this.isSpikeReset) {
                    ...
                    if (this.isSpikeGrowing) {
                        this.spikeLength += 3
                        if (this.spikeLength > spikeMax*2) {
    ...
    striker(x, y, radius = 14 + Math.ceil(Math.random() * 25)) {
        ...
        me.frictionStatic = 0;
        me.friction = 0;
        me.delay = 30 * simulation.CDScale;
        me.cd = Infinity;
        Matter.Body.rotate(me, Math.PI * 0.1);
        spawn.shield(me, x, y);
        me.onDamage = function() {
    ...
    sneaker(x, y, radius = 15 + Math.ceil(Math.random() * 10)) {
        ...
        me.do = function() {
            ...
            if (!m.isBodiesAsleep) {
                if (this.seePlayer.recall) {
                    if (this.alpha < 0.1) this.alpha += 0.003 + 0.003 / simulation.CDScale;
                } else {
        ...
        me.do = function() {
            ...
            if (!m.isBodiesAsleep) {
                if (this.distanceToPlayer2() - this.seeAtDistance2 < 0) {
                    if (this.alpha < 1) this.alpha += 0.03 * simulation.CDScale;
                } else {
                    if (this.alpha > 0) this.alpha -= 0.03;
                }
            }
    ...
    bomberBoss(x, y, radius = 88) {
        ...
        me.fireFreq = 5 + Math.floor(30 * simulation.CDScale);
                ...
                if (this.seePlayer.recall) {
                    ...
                    if (dot > threshold) {
						...
						for (let i=0; i<3; i++) {
							...
							Matter.Body.setVelocity(mob[mob.length - 1], {
								x: this.velocity.x + this.fireDir.x * v + i*3*Math.random(),
								y: this.velocity.y + this.fireDir.y * v + i*3*Math.random()
							});
						}
                        this.noseLength = 0;
                        // recoil
    ...
    launcher(x, y, radius = 30 + Math.ceil(Math.random() * 40)) {
        ...
        let me = mob[mob.length - 1];
        me.accelMag = 0.00012 * simulation.accelScale;
        me.fireFreq = Math.floor(160 + 30 * Math.random() * simulation.CDScale)
        me.frictionStatic = 0;
        me.friction = 0;
        me.frictionAir = 0.02;
        spawn.shield(me, x, y);
    ...
    launcherBoss(x, y, radius = 90) {
        ...
        me.accelMag = 0.0001 * simulation.accelScale;
        me.fireFreq = Math.floor(110 * simulation.CDScale)
        me.frictionStatic = 0;
        me.friction = 0;
        me.frictionAir = 0.02;
        me.memory = 420;
        ...
        me.do = function() {
            ...
            if (this.seePlayer.recall && !(simulation.cycle % this.fireFreq) && !m.isBodiesAsleep) {
                ...
                //fire a bullet from each vertex
                for (let i = 0, len = this.vertices.length; i < len; i++) {
                    spawn.seeker(this.vertices[i].x, this.vertices[i].y, 16)
                    //give the bullet a rotational velocity as if they were attached to a vertex
    ...
    grenadierBoss(x, y, radius = 95) {
        ...
        me.grenadeLimiter = 0
        me.onDamage = function() {
            if (this.grenadeLimiter < 360) {
                this.grenadeLimiter += 30
        ...
        me.do = function() {
            ...
            if (this.seePlayer.recall && ((this.cycle % 15) === 0) && !m.isBodiesAsleep) {
                if (this.canFire) {
                    ...
                    spawn.seeker(this.vertices[this.closestVertex1].x, this.vertices[this.closestVertex1].y, 16)
                    ...
                    spawn.seeker(this.vertices[this.closestVertex2].x, this.vertices[this.closestVertex2].y, 16)
                    Matter.Body.setDensity(mob[mob.length - 1], 0.000001); //normal is 0.001
    ...
    seeker(x, y, radius = 8, sides = 6) {
        ...
        me.do = function() {
            ...
            this.timeLimit();
			ctx.beginPath(); //draw explosion outline
            ctx.arc(this.position.x, this.position.y, 600 * (1.01 - this.timeLeft / this.lifeSpan), 0, 2 * Math.PI); //* this.fireCycle / this.fireDelay
            ctx.fillStyle = "rgba(255,0,220,0.05)";
            ctx.fill();
        };
		me.onDeath = function() {
			const pulseRadius=600
            //damage player if in range
            if (Vector.magnitude(Vector.sub(player.position, this.position)) < pulseRadius && m.immuneCycle < m.cycle) {
                m.immuneCycle = m.cycle + tech.collisionImmuneCycles; //player is immune to damage
                m.damage(0.01 * simulation.dmgScale);
            }
            simulation.drawList.push({ //add dmg to draw queue
                x: this.position.x,
                y: this.position.y,
    ...
    spawner(x, y, radius = 55 + Math.ceil(Math.random() * 50)) {
        ...
        me.onDeath = function() { //run this function on death
			if (this.mass < 2) {
				...
				for (let i = 0; i < Math.ceil(this.mass * 0.15 + Math.random() * 2.5); ++i) {
					spawn.spawner(this.position.x + (Math.random() - 0.5) * radius * 2.5, this.position.y + (Math.random() - 0.5) * radius * 2.5, (radius/2)*(Math.random()*0.3+0.7));
					Matter.Body.setVelocity(mob[mob.length - 1], {
						x: this.velocity.x + (Math.random() - 0.5) * 15,
						y: this.velocity.x + (Math.random() - 0.5) * 15
					});
    ...
    spawnOrbitals(who, radius, chance = Math.min(0.25 + simulation.difficulty * 0.005)) {
        if (Math.random() < chance) {
            ...
            const len = Math.floor(Math.min(15, 3 + Math.sqrt(simulation.difficulty))) // simulation.difficulty = 40 on hard mode level 10
            const speed = (0.007 + 0.003 * Math.random() + 0.004 * Math.sqrt(simulation.difficulty)) * ((Math.random() < 0.5) ? 1 : -1)*5
