...
const level = {
    ...
    final() {
        level.custom = () => {
			...
			player.force.x -= 0.01
			mob.forEach((x)=>{
			if (!x.isFinalBoss && !x.ignoreWind)	Body.setVelocity(x, Vector.add(x.velocity, {x:-0.033, y:0}))
			if (x.position.x < -2500) {
				x.isDropPowerUp = false
				x.leaveBody = false
				x.death()
			}
			})
			*/
			body.forEach((x)=>{
				Body.setVelocity(x, Vector.add(x.velocity, {x:-0.3, y:0}))
			})
        };
        level.customTopLayer = () => {
            ctx.fillStyle = "rgba(0,255,255,0.1)"
    ...
    intro() {
		...
		if (confirm("Go directly to final?")) {
			level.levels = ["final"]
			for (let i=0 ; i< 20; i++) {
            powerUps.spawn(2095 + 15 * (Math.random() - 0.5), -2070 - 125, "ammo", false);
			}
		}
    },
    testChamber() {
