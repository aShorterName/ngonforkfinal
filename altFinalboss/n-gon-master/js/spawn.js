...
const spawn = {
    ...
    finalBoss(x, y, radius = 320) {
        mobs.spawn(x, y, 4, radius, "rgb(150,150,255)");
		let me = mob[mob.length - 1];
        Body.scale(me, 1, 3.3)
        ...
        me.do = function() {
			...
			this.gravity();
			Body.setPosition(this, Vector.add(this.position, {x:-1, y:0}))
			if (this.lowestX < this.position.x) Body.setPosition(this, {x:this.lowestX, y: this.position.y})
			if (this.position.x < this.lowestX) this.lowestX = this.position.x
			if (player.position.x> this.position.x) m.damage(0.003 * simulation.dmgScale);
            ...
            if (this.health > 0.25) {
                if (this.cycle > this.endCycle) {
                    ...
                    if (this.mode > 3) {
                        ...
                        this.modeDo = this.bltModeList[Math.floor(Math.random() * this.bltModeList.length)]
						...
						this.modeDo = this.modeAir
					}
                }
            } else if (this.mode !== 4) { //all three modes at once
                this.cycle = 0;
        ...
        me.modeSpawns = function() {
            if (!(this.cycle % this.spawnInterval) && !m.isBodiesAsleep && mob.length < 40) {
                ...
                const whoSpawn = spawn.fullPickList[Math.floor(Math.random() * spawn.fullPickList.length)];
                    spawn[whoSpawn](this.position.x - radius - 50 * (Math.random() - 0.5), this.position.y + 50 * (Math.random() - 0.5));
                    const velocity = Vector.mult(Vector.normalise(Vector.sub(this.position, player.position)), -8)
                    Matter.Body.setVelocity(mob[mob.length - 1], {
                        x: this.velocity.x + velocity.x,
                        y: this.velocity.y + velocity.y
                    });
                if (!(this.cycle % 20 * this.spawnInterval) && !m.isBodiesAsleep && mob.length < 40) {
        ...
        me.modeSuck = function() {
            if ((this.cycle % 120) < 60) {player.force.x += 0.02} else {this.modeSpawns()}
        }
		me.modeGrenade = function() {
				if (this.cycle % 60 == 0 && !m.isBodiesAsleep) {
				mobs.spawn(this.position.x-radius, this.position.y, 12, 15, "#ff0000")
				let blt = mob[mob.length-1]
				blt.endCycle = simulation.cycle+150
				blt.isDropPowerUp = false
				blt.leaveBody = false
				blt.showHealthBar = false
				blt.damageReduction = 10
				blt.g = 0.001
				blt.do = function(){this.gravity();if (this.endCycle < simulation.cycle) this.death()}
				blt.isBadTarget = true
				blt.onHit = function() {
					this.death()
					this.explode()
				}
				blt.onDeath = function(){
					this.damageReduction = 0
					this.health = 1
					b.explosion(this.position, 300)
				}
				Body.setVelocity(blt, Vector.rotate(Vector.mult(Vector.normalise(Vector.sub(player.position,{x:this.position.x-radius, y:this.position.y})), 50), (Math.random()-0.5)/4))
			}}
		me.modeAir = function() {
				if (this.cycle % 60 == 0 && !m.isBodiesAsleep) {
				mobs.spawn(player.position.x, player.position.y-5000, 12, 15, "#ff0000")
				let blt = mob[mob.length-1]
				blt.endCycle = simulation.cycle+300
				blt.isDropPowerUp = false
				blt.leaveBody = false
				blt.showHealthBar = false
				blt.damageReduction = 10
        ...
        me.modeNail = function() {
			if (this.cycle % 10 == 0 && !m.isBodiesAsleep) {
				mobs.spawn(this.position.x-radius, this.position.y, 4, 12.5, "#ff0000")
				let nail = mob[mob.length-1]
				Matter.Body.scale(nail, 1, 0.2)
				nail.endCycle = simulation.cycle+60
				nail.isDropPowerUp = false
				nail.leaveBody = false
				nail.showHealthBar = false
				nail.ignoreWind = true
				nail.damageReduction = 10
				nail.do = function(){if (this.endCycle < simulation.cycle) this.death()}
				Body.setVelocity(nail, Vector.rotate(Vector.mult(Vector.normalise(Vector.sub(player.position,{x:this.position.x-radius, y:this.position.y})), 50), (Math.random()-0.5)/4))
			}
		}
		me.bltModeList = [me.modeNail, me.modeGrenade]
    },
